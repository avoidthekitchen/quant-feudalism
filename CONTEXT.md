...
**Active Window**:
The part of a **Compute Cycle** where **Attack Cards** are available to play through the **Attack Queues**.
_Avoid_: Player turn

**Preparing Window**:
The part of a **Compute Cycle** where attack cards are unavailable and the player survives while preparing the next active window.
...
**Attack Card**:
A drawn permission token that allows one melee or ranged attack attempt when its cost and action constraints are satisfied.
_Avoid_: Ability charge

**Card Type**:
The combat role of an **Attack Card**, such as melee or ranged.
_Avoid_: Suit
...
**Starter Deck**:
The fixed initial set of attack cards used to test whether **Compute Cycles** improve arena combat.
...
- A **Compute Cycle** contains exactly one **Active Window** and exactly one **Preparing Window**.
- An **Active Window** exposes **Attack Cards** through two **Attack Queues**: melee and ranged.
...
- The prototype **Starter Deck** contains melee and ranged **Attack Cards** but is not constructed or upgraded during a run.
...
- Ranged siphon can restore long-term Compute Credits, but it does not restore Compute Rate Limit or change the **Compute Cycle**.
- **Dash** does not consume **Attack Cards** and is not limited to one part of the **Compute Cycle**.
- During a **Preparing Window**, attacks are unavailable, movement is slowed, and Dash remains available.
- Enemies behave the same during **Active Windows** and **Preparing Windows**.
...
> **Domain expert:** "No — drawn **Attack Cards** are sorted into the melee and ranged **Attack Queues** in the HUD."

## Flagged Ambiguities

- "Turn" was used for the real-time attack-spending phase, but this can imply enemies stop acting; resolved: use **Compute Cycle** for the full cadence and **Active Window** for the attack-spending portion.
